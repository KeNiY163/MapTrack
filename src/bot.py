...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_msg = (
        ...
        "🔹 Что умеет бот:\n\n"
        "📦 Отследить контейнер - отправьте трек-номер (например: TKRU1234567) для получения информации о местонахождении контейнера\n\n"
...
async def track_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await safe_reply_text(
            update,
            "📦 Отслеживание контейнера\n\nОтправьте трек-номер контейнера (например: TKRU1234567)",
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == '📦 Отследить':
        ...
        if user_contract_history:
            ...
            await update.message.reply_text(
                "🔍 Поиск по договору\n\nОтправьте номер договора (например: 123456АБ7890)",
                reply_markup=create_reply_keyboard()
            )
        return
